...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create somm-92fb
  fly apps create somm-92fb-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app somm-92fb
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app somm-92fb-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'somm-92fb-staging' does not match app name in config file 'somm-92fb'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name somm-92fb-db
  fly postgres attach --app somm-92fb somm-92fb-db

  fly postgres create --name somm-92fb-staging-db
  fly postgres attach --app somm-92fb-staging somm-92fb-staging-db
  ```
